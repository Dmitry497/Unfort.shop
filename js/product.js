...
let fullscreenModal, fullscreenImage, fullscreenImageContainer, fullscreenClose, fullscreenPrev, fullscreenNext;
...
function openFullscreenModal(index) {
    ...
    setTimeout(() => {
        ...
        if (modalImage) {
            ...
            modalImage.addEventListener('wheel', onWheel);
            modalImage.addEventListener('touchstart', onTouchStart);
            modalImage.addEventListener('touchmove', onTouchMove);
            modalImage.addEventListener('touchend', onTouchEnd);
        }
    }, 100);
}
...
function toggleZoomOnImage() {
    ...
    if (!isZoomed) {
        ...
        img.style.transform = `scale(2) translateY(${currentTranslate}px)`;
    } else {
        img.classList.remove('zoomed');
        isZoomed = false;
        img.style.cursor = 'zoom-in';
...
function onWheel(e) {
    ...
    currentTranslate = Math.min(maxTranslate, Math.max(-maxTranslate, currentTranslate));
...
/* ========== РАСЧЁТ ДАТ ДЛЯ ДОЛЯМИ (4 ПЛАТЕЖА) ========== */
function calculateInstallmentDates() {
    ...
    dates.push({ label: 'Сегодня', date: today });
    
    let nextDate = new Date(today);
    nextDate.setDate(today.getDate() + 14);
    dates.push({ label: formatDate(nextDate), date: nextDate });
    ...
    nextDate = new Date(today);
    nextDate.setDate(today.getDate() + 42);
    dates.push({ label: formatDate(nextDate), date: nextDate });
    
    return dates;
}

function calculateInstallmentPayments(price) {
    const priceNum = parseInt(price.toString().replace(/[^\d]/g, ''));
    const paymentAmount = Math.floor(priceNum / 4);
    const remainder = priceNum - paymentAmount * 3;
    return [paymentAmount, paymentAmount, paymentAmount, remainder];
}
...
function openInstallmentModal() {
    ...
    const priceNum = parseInt(product.priceNew.toString().replace(/[^\d]/g, ''));
    const payments = calculateInstallmentPayments(priceNum);
    ...
    const payment4El = document.getElementById('payment4');
    ...
    if (payment1El) payment1El.textContent = formatPrice(payments[0]);
    if (payment2El) payment2El.textContent = formatPrice(payments[1]);
    if (payment3El) payment3El.textContent = formatPrice(payments[2]);
    if (payment4El) payment4El.textContent = formatPrice(payments[3]);
    
    if (date2El) date2El.textContent = dates[1].label;
    if (date3El) date3El.textContent = dates[2].label;
    if (date4El) date4El.textContent = dates[3].label;
...
function initInstallmentBlock() {
    const installmentBlock = document.querySelector('.product-detail__installment');
    if (installmentBlock) {
        installmentBlock.style.cursor = 'pointer';
        installmentBlock.addEventListener('click', (e) => {
            e.preventDefault();
            openInstallmentModal();
        });
    }
    
    const closeBtn = document.querySelector('.installment-modal__close');
    const overlay = document.getElementById('installmentOverlay');
    ...
    if (overlay) {
        overlay.addEventListener('click', closeInstallmentModal);
    }
}
...
function renderProductPage() {
    ...
    const priceNum = parseInt(product.priceNew.toString().replace(/[^\d]/g, ''));
    const installmentAmount = Math.floor(priceNum / 4);

    const imagesHTML = productImages.map((img, index) => `
        <div class="product-detail__image-item" data-index="${index}">
    ...
    productContainer.innerHTML = `
        <div class="product-detail">
            ...
            <div class="product-detail__info">
                ...
                <div class="product-detail__installment">Долями: 4 платежа по ${formatPrice(installmentAmount)}</div>
                ...
                <div class="product-detail__packaging">
                    <div class="product-detail__packaging-title">Выбери свою упаковку (Не влияет на стоимость)</div>
                    <div class="product-detail__packaging-options">
                        <label class="packaging-option">
                            <input type="radio" name="packaging" value="zip">
                            <span>Зип пакет</span>
                        </label>
                        <label class="packaging-option">
                            <input type="radio" name="packaging" value="vacuum">
                            <span>Вакуумный пакет</span>
                        </label>
                    </div>
                </div>
                ...
                <div class="product-detail__accordion">
                    ...
                    <div class="accordion-item">
                        <button class="accordion-header" data-target="sizes-info">
                            РАЗМЕРЫ
                            <span class="accordion-icon">+</span>
                        </button>
                        <div class="accordion-content" id="sizes-info">
                    ...
                    <div class="accordion-item">
                        <button class="accordion-header" data-target="delivery">
                            ДОСТАВКА
                            <span class="accordion-icon">+</span>
                        </button>
                        <div class="accordion-content" id="delivery">
                            <p>Бесплатная доставка от 14 990₽. Срок доставки: 7-14 дней. Отправка по всей России.</p>
                        </div>
                    </div>
                    <div class="accordion-item">
                        <button class="accordion-header" data-target="payment">
                            ОПЛАТА
                            <span class="accordion-icon">+</span>
                        </button>
                        <div class="accordion-content" id="payment">
                            <p>Оплата картами Visa, Mastercard, МИР. Также доступна оплата долями (4 платежа).</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    if (cartBtn) {
        cartBtn.addEventListener('click', (e) => {
            ...
            // Вызываем глобальную функцию addToCart из common.js
...
function initProductPage() {
    ...
    fullscreenImageContainer = document.querySelector('.fullscreen-modal__image-container');
